...
(account 0) from a testnet3 wallet listening on `localhost:18452` in several
different languages:
...
auto main() -> int {
    ...
    auto channel = grpc::CreateChannel("localhost:18452", creds);
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            var channel = new Channel("localhost:18452", new SslCredentials(cert));
            try
            {
...
var client = new walletrpc.WalletService('localhost:18452', creds);
...
def main():
    ...
    channel = implementations.secure_channel('localhost', 18452, creds)
